...
static int str_split(lua_State* L) {
    ...
    size_t l;
    size_t l2;
    const char* str = luaL_tolstring(L, 1, &l);
    const char* split = luaL_tolstring(L, 2, &l2);
    lua_newtable(L);
    if (args == 1) {
        unsigned int i;
        for (i = 0; i < l; i++) {
            if (str[i] != -51) {
                char* token = malloc(sizeof(*token) * 2);
                if (token != NULL) {
                    token[0] = str[i];
                    token[1] = 0;
                    int tableIndex = i + 1;
        ...
        for (i = 0; i < l; i++) {
            if (str[i] == split[0] && i < l) {
                ...
                if (token != NULL) {
                    token[tokenLen] = 0;
                    tokens++;
                    int maxTokenLen = tokenLen;
                    for (tokenLen; tokenLen > 0; tokenLen--) {
                        if (str[(i - tokenLen)] == -51) { token[maxTokenLen - tokenLen] = 0; }
                        else {
                            token[maxTokenLen - tokenLen] = str[(i - tokenLen)];
                        }
                    }
